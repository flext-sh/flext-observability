...
def demo_distributed_tracing(monitor: FlextObservabilityMonitor) -> None:
    ...
    if start_result.is_success:
        ...
        # Finish the trace
        finish_result = monitor._tracing_service.finish_trace("demo_trace_001", "completed")
...
def show_health_status(monitor: FlextObservabilityMonitor) -> None:
    ...
    if monitor_metrics:
        print(f"  ⏱️ Monitor uptime: {monitor_metrics.get('monitor_uptime_seconds', 0):.1f}s")
        print(f"  🔧 Functions monitored: {monitor_metrics.get('functions_monitored', 0)}")
