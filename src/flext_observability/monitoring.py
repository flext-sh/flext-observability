...
class MonitoringService:
    ...
    def record_health_check(
        self, name: str, status: str, message: str, duration_ms: float = 0.0
    ) -> None:
    ...
    def get_application_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "healthy_checks": len(
                [hc for hc in self.health_checks.values() if hc.status == "healthy"]
            ),
            "unhealthy_checks": len(
                [hc for hc in self.health_checks.values() if hc.status == "unhealthy"]
            ),
        }
...
def record_health_check(
    name: str, status: str, message: str, duration_ms: float = 0.0
) -> None:
