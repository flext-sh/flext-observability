# FLEXT CORE MONITORING - ENTERPRISE OBSERVABILITY & METRICS
...
The FLEXT Core Monitoring module provides **enterprise-grade observability** with comprehensive metrics and health checking:
...
class MetricsCollector:
    ...
    def __init__(self, registry: CollectorRegistry | None = None) -> None:
        ...
        self.app_info = Gauge("flext_app_info", "Application information", ["version", "env"])

        # System resource metrics
        self.cpu_usage = Gauge("flext_cpu_usage_percent", "Current CPU usage percent")
        self.memory_usage = Gauge("flext_memory_usage_percent", "Current memory usage percent")
        self.disk_usage = Gauge("flext_disk_usage_percent", "Disk usage percent", ["mountpoint"])

        # Pipeline execution metrics
        self.pipeline_runs_total = Counter(
            "flext_pipeline_runs_total",
            "Total number of pipeline runs",
            ["pipeline_name", "status"]
        )
        self.pipeline_duration_seconds = Histogram(
            "flext_pipeline_duration_seconds",
        ...
        self.grpc_requests_total = Counter(
            "flext_grpc_requests_total",
            "Total number of gRPC requests",
            ["method", "status"]
        )
        self.grpc_request_duration_seconds = Histogram(
            "flext_grpc_request_duration_seconds",
...
from flext_core.monitoring.metrics import MetricsCollector
...
PYTHONPATH=src python -c "
from flext_core.monitoring.health import HealthChecker
...
PYTHONPATH=src python -c "
from flext_core.monitoring.tracing import trace_async, get_tracer
