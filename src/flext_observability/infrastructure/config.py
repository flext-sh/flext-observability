...
class ObservabilitySettings(BaseSettings):
    ...
    project_version: Version = Field(
        FlextConstants.FRAMEWORK_VERSION, description="Project version",
    )
    ...
    metrics_interval: int = Field(
        15, description="Metrics collection interval (seconds)", ge=1, le=3600,
    )
    metrics_retention_days: int = Field(
        30, description="Metrics retention period (days)", ge=1, le=365,
    )

    # Tracing Configuration
    tracing_enabled: bool = Field(True, description="Enable distributed tracing")
    tracing_service_name: str = Field(
        "flext-observability", description="Service name for tracing",
    )
    tracing_service_version: str = Field(
        FlextConstants.FRAMEWORK_VERSION, description="Service version",
    )
    ...
    export_endpoint: str = Field(
        "http://localhost:4317", description="OTLP export endpoint",
    )
    export_timeout: int = Field(
        30, description="Export timeout (seconds)", ge=1, le=300,
    )
    ...
    include_trace_info: bool = Field(
        True, description="Include trace information in logs",
    )
    ...
    health_interval: int = Field(
        30, description="Health check interval (seconds)", ge=10, le=3600,
    )
    check_timeout: int = Field(
        5, description="Health check timeout (seconds)", ge=1, le=60,
    )

    # Alerting Configuration
    alerting_enabled: bool = Field(False, description="Enable alerting")
    alert_cooldown_seconds: int = Field(
        300, description="Alert cooldown period (seconds)", ge=60, le=3600,
    )
    ...
    webhook_timeout: int = Field(
        30, description="Webhook timeout (seconds)", ge=1, le=300,
    )
    ...
    dashboard_refresh_interval: int = Field(
        30, description="Dashboard refresh interval (seconds)", ge=5, le=300,
    )

    # Performance thresholds
    cpu_threshold_percent: float = Field(
        80.0, description="CPU usage threshold (%)", ge=0.0, le=100.0,
    )
    memory_threshold_percent: float = Field(
        80.0, description="Memory usage threshold (%)", ge=0.0, le=100.0,
    )
    disk_threshold_percent: float = Field(
        80.0, description="Disk usage threshold (%)", ge=0.0, le=100.0,
    )
    response_time_threshold_ms: int = Field(
        1000, description="Response time threshold (ms)", ge=1, le=60000,
    )
