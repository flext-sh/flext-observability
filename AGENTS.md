...
Enterprise monitoring, metrics & telemetry. Used by the Singer taps/targets for run
telemetry.
...
- Metric type/ID/labels are normalized by private helpers **before** model creation —
  construct through the facade, not the models directly.
- Structured logging flows through the `flext-core` logger factory; never call
