...
- [Core observability foundation](#core-observability-foundation)
- [All ecosystem projects start with observability patterns here](#all-ecosystem-projects-start-with-observability-patterns-here)
  - [**Domain Entity Layer**](#domain-entity-layer)
- [Core observability domain models](#core-observability-domain-models)
  - [**Application Services Layer**](#application-services-layer)
- [Observability business logic services](#observability-business-logic-services)
  - [**Factory & Creation Layer**](#factory--creation-layer)
- [Entity creation and factory patterns](#entity-creation-and-factory-patterns)
  - [**Interface Adapters Layer**](#interface-adapters-layer)
- [External interface adaptation](#external-interface-adaptation)
- [Decorator automatically:](#decorator-automatically)
- [- Creates execution time metrics](#--creates-execution-time-metrics)
- [- Creates trace spans with operation context](#--creates-trace-spans-with-operation-context)
- [- Handles success/failure tracking](#--handles-successfailure-tracking)
- [- Logs structured entries with correlation IDs](#--logs-structured-entries-with-correlation-ids)
...
- [Observability entities](#observability-entities)
...
- [Module names focus on observability concerns](#module-names-focus-on-observability-concerns)
  - [**Function Naming Patterns**](#function-naming-patterns)
- [Simple API functions use flext*create* prefix](#simple-api-functions-use-flext_create_-prefix)
- [Monitoring functions use flext*monitor* prefix](#monitoring-functions-use-flext_monitor_-prefix)
- [Factory access functions](#factory-access-functions)
...
- [Import from main package - gets essential observability tools](#import-from-main-package---gets-essential-observability-tools)
- [Use patterns directly in business logic](#use-patterns-directly-in-business-logic) - [**2. Service Integration Pattern (For FLEXT Services)**](#2-service-integration-pattern-for-flext-services)
- [Import services for advanced integration](#import-services-for-advanced-integration) - [**3. Infrastructure Integration Pattern (For Infrastructure Services)**](#3-infrastructure-integration-pattern-for-infrastructure-services)
- [Import for infrastructure monitoring](#import-for-infrastructure-monitoring)
  - [**Anti-Patterns (Forbidden)**](#anti-patterns-forbidden)
- [❌ Don't import everything](#-dont-import-everything)
- [❌ Don't import internal implementations](#-dont-import-internal-implementations)
- [❌ Don't bypass simple API without reason](#-dont-bypass-simple-api-without-reason)
- [❌ Don't create custom observability types](#-dont-create-custom-observability-types)
- [❌ Don't ignore FlextResult error handling](#-dont-ignore-flextresult-error-handling)
  - [🏛️ **Architectural Patterns**](#-architectural-patterns)
    - [**Layer Separation**](#layer-separation)
- [Observability architecture with clear boundaries](#observability-architecture-with-clear-boundaries)
  - [**Dependency Direction**](#dependency-direction)
- [Dependencies flow inward (Clean Architecture)](#dependencies-flow-inward-clean-architecture)
  - [**Cross-Cutting Observability Concerns**](#cross-cutting-observability-concerns)
- [Handled via decorators and context management](#handled-via-decorators-and-context-management)
  - [🔄 **Observability-Specific Patterns**](#-observability-specific-patterns)
    - [**Metric Creation Patterns**](#metric-creation-patterns)
- [Basic metric creation](#basic-metric-creation)
- [Advanced metric patterns with validation](#advanced-metric-patterns-with-validation)
  - [**Distributed Tracing Patterns**](#distributed-tracing-patterns)
- [Parent-child trace correlation](#parent-child-trace-correlation)
...
- [Business rule-based alerting](#business-rule-based-alerting)
- [Alert escalation patterns](#alert-escalation-patterns)
  - [🧪 **Testing Patterns**](#-testing-patterns)
    - [**Test Organization**](#test-organization)
- [Test structure mirrors observability structure](#test-structure-mirrors-observability-structure)
...
- [✅ Complete type annotations for observability functions](#-complete-type-annotations-for-observability-functions)
- [✅ Generic type usage for observability utilities](#-generic-type-usage-for-observability-utilities)
- [✅ Protocol definitions for observability interfaces](#-protocol-definitions-for-observability-interfaces)
- [❌ Missing type annotations](#-missing-type-annotations)
  - [**Error Handling Standards**](#error-handling-standards)
- [✅ Always use FlextResult for observability error handling](#-always-use-flextresult-for-observability-error-handling)
- [✅ Chain observability operations safely](#-chain-observability-operations-safely)
- [❌ Never raise exceptions for observability failures](#-never-raise-exceptions-for-observability-failures)
...
- [✅ Standard observability imports across ecosystem](#-standard-observability-imports-across-ecosystem)
- [✅ Consistent observability patterns across FLEXT projects](#-consistent-observability-patterns-across-flext-projects)
- [✅ Cross-service observability correlation](#-cross-service-observability-correlation)
- [❌ Don't create service-specific observability types](#-dont-create-service-specific-observability-types)
  - [**Configuration Integration Across Services**](#configuration-integration-across-services)
- [✅ Extend observability configuration patterns](#-extend-observability-configuration-patterns)
- [Usage in service initialization](#usage-in-service-initialization)
  - [**Monitoring Integration Patterns**](#monitoring-integration-patterns)
- [✅ Consistent monitoring across ecosystem services](#-consistent-monitoring-across-ecosystem-services)
...
def process_order(order_data: dict) -> FlextResult[FlextTypes.Dict]:
...
class UserAPIService:
    ...
    def handle_user_request(self, request: dict) -> FlextResult[FlextTypes.Dict]:
...
class DatabaseConnectionService:
    ...
    def check_database_connectivity(self) -> FlextResult[FlextTypes.Dict]:
...
def process_payment(payment_data: dict) -> FlextResult[FlextTypes.Dict]:
...
def process_order_with_tracing(order_data: dict) -> FlextResult[FlextTypes.Dict]:
...
def validate_order_with_trace(order_data: dict, parent_trace_id: str) -> FlextResult[FlextTypes.Dict]:
...
def monitor_service_health() -> FlextResult[FlextTypes.Dict]:
...
def test_observability_chaining():
    ...
    def create_observability_data(operation: str) -> FlextResult[FlextTypes.Dict]:
        return (
...
def test_observability_failure_propagation():
    ...
    def create_invalid_observability() -> FlextResult[FlextTypes.Dict]:
        return (
...
def monitor_business_operation(operation_data: dict) -> FlextResult[FlextTypes.Dict]:
...
def sync_user_between_services(user_id: str) -> FlextResult[FlextTypes.Dict]:
...
class FlextAPIService:
    ...
    def query_user_data(self, user_id: str) -> FlextResult[FlextTypes.Dict]:
...
class FlextOracleService:
    ...
    def execute_wms_query(self, query: str) -> FlextResult[list[FlextTypes.Dict]]:
...
class FlextLdapService:
    ...
    def search_users(self, search_filter: str) -> FlextResult[list[FlextTypes.Dict]]:
